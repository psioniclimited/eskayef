...
function sanitize_user_field($field, $value, $user_id, $context) {
	...
	if ( 'edit' == $context ) {
		if ( $prefixed ) {
			 ...
			 * field being filtered, such as 'user_login', 'user_email', 'name', etc.
			 *
			 * @since 2.9.0
			 *
...
function wp_insert_user( $userdata ) {
	...
	$first_name = empty( $userdata['name'] ) ? '' : $userdata['name'];
	 ...
	 */
	$meta['name'] = apply_filters( 'pre_user_first_name', $first_name );
	...
	if ( empty( $userdata['display_name'] ) ) {
		...
		} elseif ( $meta['name'] && $meta['last_name'] ) {
			/* translators: 1: first name, 2: last name */
			$display_name = sprintf( _x( '%1$s %2$s', 'Display name based on first name and last name' ), $meta['name'], $meta['last_name'] );
		} elseif ( $meta['name'] ) {
...
function _get_additional_user_keys( $user ) {
	$keys = array( 'name', 'last_name', 'nickname', 'description', 'rich_editing', 'comment_shortcuts', 'admin_color', 'use_ssl', 'show_admin_bar_front', 'locale' );
